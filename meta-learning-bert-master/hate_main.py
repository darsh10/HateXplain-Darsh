...
from hate_task import MetaTask, TestMetaTask
import random
import numpy as np
import pdb
...
def main():
    ...
    parser.add_argument("--data", default='hate_dataset.json', type=str,
    ...
    low_resource_domains = ["Women"]#, "Homosexual", "Christian", "Hindu", "Bisexual", "Indian"]
    high_resource_domains= set([r['domain'] for r in reviews \
            if r['domain'] not in low_resource_domains])
    ...
    test = TestMetaTask(test_examples, num_task = args.num_task_test,
            k_support=args.k_spt, k_query=args.k_qry, tokenizer = tokenizer)
