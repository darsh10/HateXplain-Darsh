...
class MetaTask(Dataset):
    ...
    def create_batch(self, num_task):
        ...
        for e in self.examples:
            if e['split'] == 'train':
                domain_train_counts[e['domain']] = domain_train_counts.setdefault(\
                        e['domain'],0) + 1
            if e['split'] == 'test':
                domain_test_counts[e['domain']]  = domain_test_counts.setdefault(\
                        e['domain'],0) + 1
        for d in domain_train_counts.keys():
            if domain_train_counts.get(d,0) > self.k_support and\
                    domain_test_counts.get(d,0) > self.k_query:
                acceptable_domains.append(d)

        for b in range(num_task):  # for each task
            # 1.select domain randomly
            domain = random.choice(acceptable_domains)
...
class TestMetaTask(Dataset):
    ...
    def create_batch(self, num_task):
        ...
        domains = list(set([e['domain'] for e in self.examples]))
        for domain in domains[:self.num_task]:  # for each task

            exam_train = [e for e in self.examples \
                    if e['domain'] == domain and e['split'] == 'train']
            exam_test  = [e for e in self.examples \
                    if e['domain'] == domain and e['split'] == 'test']
